...
class SOED(object):
    ...
    def get_sPCE_reward(self, stage, L, thetas0, d_hist, y_hist, xp_hist):
        ...
        stage : int
            The stage index. 0 <= stage <= n_stage.
        L : int
            The number of contrastive samples to be drawn to estimate the sPCE
            lower bound.
        thetas0 : numpy.ndarray of size (1, n_param)
        d_hist : numpy.ndarray of size (n_stage, n_design)
            The sequence of designs from stage 0.
        y_hist : numpy.ndarray of size (n_stage, n_obs)
            The sequence of observations from stage 0.
        xp_hist : numpy.ndarray of size (n_stage + 1, n_phys_state)
            The sequence of physical states from stage 0.

        Returns
        -------
        A float value which is the total sPCE lower bound reward.
        """
        # avoid calculating the reward unless final stage of the experiment
        if stage < self.n_stage:
            ...
            # draw L contrastive samples from the prior of theta
            thetas_sPCE[1:] = self.prior_rvs(L) 
            post_thetas = self.post_pdf(thetas_sPCE, stage=0, d_hist=d_hist, 
                                  y_hist=y_hist, xp_hist=xp_hist, 
                                  include_prior=False)

            # calculate the sPCE lower bound
            reward = np.log(post_thetas[0] / (np.sum(post_thetas) / (L+1)) + 1e-32)
